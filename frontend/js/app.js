...
async function showGroupMembers(groupId) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const action = prompt('用户组: ' + group.name + '\n成员: ' + memberList + '\n\n请输入操作:\n1. 添加成员 (输入用户名)\n2. 移除成员 (输入 -用户名)');
        ...
        if (targetType === 'user') {
            ...
            if (response.ok) {
                select.innerHTML = data.users.map(u => 
                    '<option value="' + u.id + '">' + escapeHtml(u.username) + (u.group_name ? ' (' + u.group_name + ')' : '') + '</option>'
                ).join('');
            }
        } else {
            const response = await fetch(API_BASE + '/groups', {
            ...
            if (response.ok) {
                select.innerHTML = data.groups.map(g => 
                    '<option value="' + g.id + '">' + escapeHtml(g.name) + ' (' + g.member_count + '人)</option>'
                ).join('');
            }
        }
    } catch (error) {
...
renderUsers = function(users) {
    ...
    tbody.innerHTML = users.map(user => '
        <tr>
            ...
            <td>
                <button class="btn-action btn-edit" onclick="editUser(' + user.id + ')" title="编辑">✏️</button>
                <button class="btn-action btn-toggle" onclick="toggleUser(' + user.id + ', ' + user.is_active + ')" title="' + (user.is_active ? '停用' : '启用') + '">' + (user.is_active ? '🚫' : '✅') + '</button>
                <button class="btn-action btn-reset" onclick="resetPassword(' + user.id + ')" title="重置密码">🔑</button>
